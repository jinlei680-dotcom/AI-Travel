...
### 提示词约束与后端处理方案（生成行程与预算）

**总体思路**
- 采用“强约束提示词 + 后端多步校验与补全”的混合方式：模型负责在约束内编排路线与文案；后端负责结构化校验、地图/预算数据增强、回退与修复，确保输出严格契合前端契约。

**提示词约束设计**
- 输出格式与契约：要求仅输出 JSON，严格遵循 `Itinerary` 结构（天/项目/交通/费用等），禁止额外文本或注释。
- JSON Schema 限定：在系统/开发者消息中嵌入完整 Schema；声明偏离 Schema 的输出视为失败并丢弃；规范时间与枚举（如 `HH:mm`、`type=sight|food|transport|hotel|other`）。
- 候选 POI 提供：后端预检目的地与兴趣相关 POI 候选（名称/坐标/评分/开放时间/人均），提示词要求仅从候选中选择，避免坐标与价格幻觉。
- 约束策略提示：节奏（轻松/标准/紧凑）约束每日项目数与移动半径；兴趣权重与预算区间作为硬约束；信息缺失需在 `notes` 明确“待确认/需预约”。
- 示例与自检：提供 1–2 个 Few-shot 样例；在提示结尾加入自检清单（字段完整性、时间不重叠、预算不超出、仅使用候选 POI、JSON 校验通过）。

**后端处理管线**
- 规范化输入：解析 `PlanSpec`（目的地/日期/偏好/人数），计算天数与每日时段槽位，将节奏与兴趣映射为约束参数。
- 候选检索与预处理：调用 `GET /api/map/search` 生成 POI 候选，清理重复与低评分，附加开放时间与人均；按兴趣/距离/评分打分。
- 受控生成：将候选 POI、时段槽位、偏好约束与 JSON Schema 一并传递至模型；可采用“分日生成”或“骨架→填充”两段式，降低错误率。
- 校验与自愈：用 Zod/JSON Schema 验证；轻微错误（时间/枚举）直接修正或二次小提示修复；严重错误（JSON 失败/缺关键字段/未用候选）触发重试或保守回退。
- 地图与路线增强：对相邻项目调用 `GET /api/map/route` 获取 `polyline/duration/distance`；生成当天或全局 `routePath[]` 与分类着色 `markers[]`。
- 预算估算与汇总：按餐饮人均、门票、交通、住宿预算区间估算；汇总 `summary.total/by_category/by_day`，缺失值用 `notes` 说明。
- 持久化与版本：写库到 `trip_plans`、`itinerary_days/items`，保存 `preferences` 与版本快照；支持“锁定项”以便局部微调不改动。
- 回退与降级：模型连续失败时，回退到规则/启发式组装（基于候选打分与距离/时间约束），确保至少返回可用骨架与基本地图数据。

**可靠性与体验增强**
- 结构化输出控制：启用供应商的严格 JSON 输出模式并降低温度；必要时流式分块（先骨架后补充）。
- 审核与纠错回路：引入“批判者”二次检查草案（时间重叠/预算超标/路径跳跃/候选未用），输出修正建议并合并为最终稿。
- 性能与成本：按目的地+日期+偏好哈希做结果缓存；POI与路线缓存；限制候选数量与路线计算次数。
- 监控与告警：记录结构校验失败、重试与回退比率；异常集中时降低复杂度或更换模型策略。

**局部微调与锁定策略**
- 全局再生成：按当前偏好重排所有天，保留已锁定项。
- 局部微调：仅某天或某一时段生成；在提示中传入 `locked_item_ids` 与时间段，后端校验确保未改动锁定项。
- 变更影响标注：返回中附带影响范围说明，前端用于用户提示。

**费用预算与记账（核心能力）**
- 预算生成：后端根据类别与供应商价格规则计算费用与汇总，模型用于文案与标签辅助。
- 记账解析：`POST /api/expense/parse` 走 JSON Schema + 校验 + 汇率转换与分类统计，确保与预算总览一致。

**可替代方案（更强鲁棒性）**
- 规则引擎 + 模型小任务：用规则引擎做编排与估算，LLM 负责文案润色与标签分类，提高可控性。
- 多候选投票与裁决：生成多个低复杂度草案，评审选择最优或融合，降低单次幻觉影响。
- 工具优先的函数调用：模型通过函数取得 POI/路线/价格，最终仅聚合工具返回值，避免凭空编造。

注：该方案确保返回满足 `docs/FrontendDesign.md` 的前端契约，并覆盖“个性化路线 + 费用预算与管理”的核心能力。
